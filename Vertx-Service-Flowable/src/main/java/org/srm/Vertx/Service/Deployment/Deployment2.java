...
public class Deployment2 {
	static Logger log = LoggerFactory.getLogger(Deployment2.class);
	...
	public static void main(String[] args) {
		...
		Vertx.clusteredVertx(options, cluster -> {
			if (cluster.succeeded()) {
				cluster.result().deployVerticle(FlowableStartEvent.class.getName(),
						new DeploymentOptions().setInstances(1).setWorker(true), res -> {
							if (res.succeeded()) {
								log.info(FlowableStartEvent.class.getSimpleName() + " Deployment Suceeded.");
